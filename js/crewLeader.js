...
export class CrewLeader {
    ...
    async callAI(prompt, data = null, retries = 2) {
        ...
        if (apiKey && typeof apiKey === 'string' && apiKey.trim().length > 10) {
            for (let attempt = 0; attempt <= retries; attempt++) {
                try {
                    ...
                    console.error(`Gemini API call failed (attempt ${attempt+1}):`, e);
                    if (attempt === retries) return null;
                    await sleep(500 * (attempt + 1));
                }
            }
    ...
    async synthesize(goal, agentResults) {
        ...
        if (agentResults.TaxWizard) {
            ...
            summary += `✅ Tax Wizard: The ${tax.better} regime saves you ${fmt(tax.saving)} per year. `;
            if (tax.missedDeductions.length) summary += `Consider these missed deductions: ${tax.missedDeductions.join(', ')}. `;
        }
        if (agentResults.FIREPlanner) {
            const fire = agentResults.FIREPlanner;
            summary += `✅ FIRE Planner: You need to invest ${fmtL(fire.monthlySIP)} monthly for ${fire.yearsToRetire} years to reach your retirement goal. `;
            if (!fire.feasible) summary += `This exceeds your current surplus – consider reducing expenses or extending your timeline. `;
        }
        if (agentResults.MFXRay) {
            const mf = agentResults.MFXRay;
            summary += `✅ MF X‑Ray: Your portfolio XIRR is ${mf.avgXirr.toFixed(1)}%. `;
            if (mf.overlapping.length) summary += `Reduce overlap in ${mf.overlapping.map(o => o[0]).join(', ')} to save on expense ratios. `;
        }
        if (agentResults.RiskAssessor) {
            const risk = agentResults.RiskAssessor;
            summary += `✅ Risk Assessor: Concentration risk is ${(risk.concentration * 100).toFixed(1)}%. `;
            if (risk.riskLevel === 'High') summary += `Consider diversifying across more funds. `;
        }
        if (agentResults.TaxAwareRebalancer) {
            const switches = agentResults.TaxAwareRebalancer;
            if (switches.length) summary += `✅ Tax‑Aware Rebalancer: Switching ${switches.map(s => s.from).join(', ')} could save up to ${fmt(switches[0]?.savings || 0)}/yr. `;
        }
        if (agentResults.ResearchAgent) {
            const research = agentResults.ResearchAgent;
            summary += `✅ Research Agent: Overall sentiment for ${research.overallSentiment} – suggested funds: ${research.suggestedFunds.join(', ')}. `;
        }
        if (!summary) summary = "No agent results available. Please run agents individually first.";
        const prompt = `User goal: "${goal}".\nAgent outputs summary:\n${summary}\n\nProvide a concise, actionable plan in 2-3 sentences.`;
        const aiResponse = await this.callAI(prompt);
        if (aiResponse && aiResponse !== "⚠️ AI unavailable. Showing rule-based insights.") {
            return aiResponse;
        }
        return summary;
    }

    async askForHoldings() {
        return new Promise((resolve) => {
            this.holdingsResolve = resolve;
                ...
                if (agent === 'TaxWizard') {
                    html += `<div style="margin-bottom: 1rem; border-bottom: 1px solid var(--border); padding-bottom: 0.5rem;">
                        ...
                        <div class="form-grid" style="margin-top: 0.5rem;">
                            ...
                            <div class="field"><label>City Type</label><select id="q_t_city"><option value="metro" ${document.getElementById('t-city')?.value === 'metro' ? 'selected' : ''}>Metro</option><option value="nonmetro">Non-Metro</option></select></div>
                        </div>
                    </div>`;
                }
    ...
    async runMFXRay(silent = false) {
        if (window.holdings && window.holdings.length === 0) {
            if (!silent) {
                const choice = await this.askForHoldings();
                if (choice === 'load') {
                    window.loadMFScenario();
                    await sleep(500);
                    return this.runMFXRay(silent);
                } else if (choice === 'add') {
                    window.openAddFundModal();
                    let waited = 0;
                    while (window.holdings.length === 0 && waited < 30) {
                        await sleep(500);
                        waited++;
                    }
                    if (window.holdings.length > 0) return this.runMFXRay(silent);
                    else return null;
                } else return null;
            } else return null;
        }
        ...
        if (window.holdings && window.holdings.length === 0) {
            if (!silent) {
                ...
                if (choice === 'load') {
                    window.loadMFScenario();
                    await sleep(500);
                    return this.runRiskAssessor(silent);
                } else if (choice === 'add') {
                    window.openAddFundModal();
                    let waited = 0;
                    while (window.holdings.length === 0 && waited < 30) {
                    ...
                    if (window.holdings.length > 0) return this.runRiskAssessor(silent);
                    else return null;
                } else return null;
            } else return null;
        }
        ...
        if (!silent) {
            ...
            if (div) {
                div.innerHTML = `<div class="output-card">
                    ...
                    <div class="card-body">
                        <div><strong>Concentration Risk:</strong> ${(res.concentration * 100).toFixed(1)}% of portfolio in a single fund</div>
                        <div><strong>Risk Level:</strong> ${res.riskLevel}</div>
                        <div><strong>Diversification Score:</strong> ${res.diversificationScore} / 10</div>
        ...
        if (window.holdings && window.holdings.length === 0) {
            if (!silent) {
                ...
                if (choice === 'load') {
                    window.loadMFScenario();
                    await sleep(500);
                    return this.runRebalancer(silent);
                } else if (choice === 'add') {
                    window.openAddFundModal();
                    let waited = 0;
                    while (window.holdings.length === 0 && waited < 30) {
                    ...
                    if (window.holdings.length > 0) return this.runRebalancer(silent);
                    else return null;
                } else return null;
            } else return null;
        }
        ...
        if (!silent) {
            ...
            if (div) {
                div.innerHTML = `<div class="output-card"><div class="card-header">🔍 Research: ${topic}</div><div class="card-body">
                    ...
                    <div><strong>Headlines:</strong><ul>${res.headlines.map(h => `<li><a href="${h.url}" target="_blank" style="color:var(--blue); text-decoration:none;">${h.text}</a> (${h.sentiment})</li>`).join('')}</ul></div>
