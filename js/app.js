...
// UI functions exposed to global for inline onclick handlers
...
window.resolveHoldingsQuiz = (choice) => ui.resolveHoldingsQuiz(choice, crewLeader);
window.addCustomFund = () => {
    ...
    holdings.push({ name, category, invested, currentNav: nav, units, purchaseDate: date, expenseRatio, topStocks });
    renderHoldings();
...
window.removeHolding = (i) => { holdings.splice(i, 1); renderHoldings(); };
...
window.loadMFScenario = () => {
    ...
    holdings = SCENARIO_FUNDS.map(f => ({ ...f, topStocks: [...f.topStocks] }));
    renderHoldings();
};
...
// Expose holdings globally for agents to access
...
// Make audit and other globals available to modules
window.audit = audit;
